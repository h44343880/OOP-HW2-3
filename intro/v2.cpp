...
class TRA_switch: public node {
        ...
        map<unsigned int, pair<unsigned int, int>> table; // store <dst_node, <nex_node, distance>>
...
void TRA_switch::recv_handler (packet *p){
    ...
    if (p->type() == "TRA_ctrl_packet") { // the switch receives a packet from the controller
        ...
        // const map<unsigned int,bool> &nblist = getPhyNeighbors(); // get every neighbor nodes' id
        unsigned int src = p3->getHeader()->getSrcID(), pre = p3->getHeader()->getPreID(); //get source node id, previous node id
        int cnt = l3->getCounter(); // get count
        if((table.find(src) == table.end()) || (table[src].second == cnt && table[src].first > pre)){
            table[src] = {pre, cnt};
        ...
        // else if(table[src].second == cnt && table[src].first > pre){
        //     // cout<<"dst: "<<src<<" distance: "<<table[src].second<<" next_node: "<<table[src].first<<endl;
        //     table[src] = {pre, cnt};
        //     p3->getHeader()->setPreID ( getNodeID() );
        //     p3->getHeader()->setNexID ( BROCAST_ID );
        //     p3->getHeader()->setDstID ( BROCAST_ID );
        ...
        if(table.find(p3->getHeader()->getDstID()) == table.end() || getNodeID() == p3->getHeader()->getDstID()){
            return;
        }
        p3->getHeader()->setPreID ( getNodeID() );
...
int main()
{
    ...
    int nodes, dsts, links, pairs, sim_time;
    cin>>nodes>>dsts>>links>>pairs>>sim_time;
    ...
    vector<pair<unsigned int, unsigned int>> brocast_time;
    for(int i = 0; i < dsts; i++){
        cin>>input;
        dst_list.push_back(input);
    }
    unsigned int id;
    for(int i = 0; i < dsts; i++){
        cin>>id>>input;
        brocast_time.push_back({id, input});
    }
    ...
    for(int i = 0; i < dsts; i++){
        TRA_ctrl_packet_event(brocast_time[i].first, brocast_time[i].second);
    }
    // 1st parameter: the source; the destination that want to broadcast a msg with counter 0 (i.e., match ID)
    // 2nd parameter: time (optional)
    // 3rd parameter: msg for debug information (optional)
    ...
    int flow_size, start_time;
    for(int i = 0; i < pairs; i++){
        cin>>id>>src>>dst>>flow_size>>start_time;
        data_packet_event(src, dst, start_time);
    }
    ...
    for(int i = 0; i < nodes; i++){
        cout<<i<<endl;
        node * temp = node::id_to_node(i);
        TRA_switch * ptr = dynamic_cast<TRA_switch*> (temp);
        ptr->print_table(); 
    }
    return 0;
}
